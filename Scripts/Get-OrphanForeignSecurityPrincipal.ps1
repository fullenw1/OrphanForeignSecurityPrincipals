Function Get-OrphanForeignSecurityPrincipal
{
    ...
    Process
    {
        ...
        foreach($ForeignSecurityPrincipal in $ForeignSecurityPrincipalList)
        {
            ...
            {
                $FSPTranslation=(New-Object System.Security.Principal.SecurityIdentifier($ForeignSecurityPrincipal.objectSid)).Translate([System.Security.Principal.NTAccount])
            ...
            Catch #Find matching domain name
            {
                ...
                Write-Debug -Message "Could not resolve $DistinguishedName"
...
Function Find-AllDomainsAndSID
{
    ...
    Process
    {
        ...
        $AllTrustedDomains=Get-ADTrust -Filter *

        $DomainSIDList=@{}

        foreach($objDomain in $AllTrustedDomains)
        {
            $DomainFullName=$objDomain.Name
            ...
            Try
            {
                $DomainSID=(Get-ADDomain -Identity $DomainFullName).DomainSID
            }
            Catch
            {
                $DomainSID='Domain SID resolution failed'
            }
